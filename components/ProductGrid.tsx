...
const ProductGrid: React.FC<ProductGridProps> = ({ products, onAddToCart, onViewProduct }) => {
  const getMinPrice = (product: Product) => {
    ...
    if (variants.length === 0) return { min: product.price, hasDiscount: false, oldMin: null };
    const prices = variants.map(v => v.discountPrice || v.price || 0);
    const oldPrices = variants.map(v => v.price || 0);
    const min = Math.min(...prices);
    const oldMin = Math.min(...oldPrices);
    const hasDiscount = variants.some(v => v.discountPrice && v.discountPrice < (v.price || 0));
    return { min, hasDiscount, oldMin: hasDiscount ? oldMin : null };
  };
            ...
            >
              <div className="relative aspect-[4/5] overflow-hidden bg-gray-50">
                {firstImage ? (
                  ...
                  <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-300 text-xs font-bold">
                    Şəkil yoxdur
                  </div>
                )}
                {hasDiscount && (
