...
const Footer: React.FC<FooterProps> = ({ onReviewsClick }) => {
  return (
    <footer style={{ background: '#111111', color: '#FFFFFF', padding: '72px 32px 32px', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 48, marginBottom: 64 }}>
          ...
          <div>
            ...
            {[
              { label: 'Məhsullar', href: null, action: undefined },
              { label: 'Müştəri rəyləri', href: null, action: onReviewsClick },
              { label: 'WhatsApp sifariş', href: 'https://wa.me/994519831483', action: undefined },
            ].map((link, i) => (
              <div key={i} style={{ marginBottom: 12 }}>
                {link.href ? (
                  <a href={link.href} style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, textDecoration: 'none', fontWeight: 400, display: 'block', transition: 'color 0.15s' }}
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: 12 }}>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', fontWeight: 400 }}>© 2025 Ravio. Bütün hüquqlar qorunur.</span>
