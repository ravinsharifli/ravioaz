            ...
            <div className="p-5 md:p-7 space-y-3">
              ...
              <div className="bg-amber-50 border border-amber-200 p-3 rounded-2xl flex items-start gap-2">
                ...
                <p className="text-[11px] font-bold text-amber-800 leading-relaxed">
                  <span className="font-black">Özəl hazırlanmış məhsul!</span> Sifariş zamanı <span className="font-black text-amber-900">50% ön ödəniş</span> kart-karta alınır. Qalan məbləğ çatdırılma zamanı ödənilir.
                </p>
              </div>

              {/* ── BLOK 4: Rəng seçimi — kiçik, tək sıra ── */}
              {hasColors && (
                <div className="space-y-1.5">
                  ...
                  {isCompletelyOutOfStock && (
                    <p className="text-[11px] font-black text-red-500">❌ Bu məhsul bitib</p>
                  )}
                  {/* Tək sıra — overflow scroll əgər çox rəng varsa */}
                    ...
                    {colorVariants.map((variant) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => { if (!isVariantOutOfStock) setSelectedColor(isChecked ? '' : variant.colorName); }}
                          disabled={isVariantOutOfStock}
                          className={`flex-shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border-2 transition-all text-xs font-black whitespace-nowrap ${
                            isVariantOutOfStock
                              ? 'border-gray-100 bg-gray-50 text-gray-300 cursor-not-allowed line-through'
                              : isChecked
                              ? 'border-[#FF8C00] bg-orange-50 text-[#FF8C00]'
                              : 'border-gray-200 bg-white text-[#1A1A1A] hover:border-orange-300'
                          }`}
                        >
                          {/* mini checkbox */}
                          ...
                          <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${
                            ...
                            {isVariantOutOfStock ? 'Bitib' : isLowStock ? `Son ${variant.stock} əd` : `Stokda var: ${variant.stock} əd`}
                          </span>
                        </button>
                      );
                    })}
                  </div>
                  {isSelectedColorOutOfStock && <p className="text-[11px] font-black text-red-500">❌ Bu rəng bitib, digər rəng seçin</p>}
                  {selectedColor && !isSelectedColorOutOfStock && selectedColorStock !== null && selectedColorStock < 5 && (
                    <p className="text-[11px] font-black text-yellow-600">⚠️ {selectedColor} rəngindən yalnız {selectedColorStock} əd qalıb!</p>
                  )}
                </div>
              )}

              {/* ── BLOK 5: Endirim seçimi ── */}
              <section
                id="customer-type-section"
                className={`p-3.5 rounded-[1.5rem] space-y-2 border-2 transition-all ${showCustomerTypeError && customerType === null ? 'border-red-400 bg-red-50' : 'border-orange-200 bg-gradient-to-br from-orange-50 to-white'}`}
              >
                <div className="flex items-center gap-2">
                  <Sparkles className="h-3.5 w-3.5 text-[#FF8C00]" />
                  <h4 className="text-xs font-black text-[#1A1A1A]">Sizə özəl endirim — seçin:</h4>
                </div>
                <div className="flex gap-2">
                  <label onClick={() => { setCustomerType('new'); setShowCustomerTypeError(false); }}
                    className={`flex-1 flex items-center gap-2 p-3 rounded-2xl border-2 cursor-pointer transition-all ${customerType === 'new' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-200 bg-white hover:border-orange-200'}`}>
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${customerType === 'new' ? 'border-[#FF8C00]' : 'border-gray-300'}`}>
                      {customerType === 'new' && <div className="w-2 h-2 rounded-full bg-[#FF8C00]" />}
                    </div>
                    <div>
                      <p className="text-[11px] font-black text-[#1A1A1A]">Yeni müştəri</p>
                      <p className="text-[10px] font-bold text-[#FF8C00]">10% endirim 🎉</p>
                    </div>
                  </label>
                  <label onClick={() => { setCustomerType('loyal'); setShowCustomerTypeError(false); }}
                    className={`flex-1 flex items-center gap-2 p-3 rounded-2xl border-2 cursor-pointer transition-all ${customerType === 'loyal' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-200 bg-white hover:border-orange-200'}`}>
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${customerType === 'loyal' ? 'border-[#FF8C00]' : 'border-gray-300'}`}>
                      {customerType === 'loyal' && <div className="w-2 h-2 rounded-full bg-[#FF8C00]" />}
                    </div>
                    <div>
                      <p className="text-[11px] font-black text-[#1A1A1A]">Daimi müştəri</p>
                      <p className="text-[10px] font-bold text-[#FF8C00]">10% endirim ❤️</p>
                    </div>
                  </label>
                </div>
                {showCustomerTypeError && customerType === null && (
                  <p className="text-[11px] font-black text-red-500">⚠️ Zəhmət olmasa müştəri növünü seçin</p>
                )}
              </section>

              {/* ════════════════════════════════
                  FORM SAHƏLƏRİ — hamısı ardıcıl
              ════════════════════════════════ */}
              <div className="border-t-2 border-dashed border-gray-100 pt-4 space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl text-sm min-h-[52px] outline-none text-[#1A1A1A] font-bold placeholder:text-gray-300 focus:border-orange-200 transition-all resize-none"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl text-sm min-h-[60px] outline-none text-[#1A1A1A] font-bold placeholder:text-gray-300 focus:border-orange-200 transition-all resize-none"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" placeholder="Adınız" value={customerName} onChange={(e) => setCustomerName(e.target.value)}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-100 focus:border-orange-200 rounded-xl outline-none font-bold text-sm placeholder:text-gray-300" />
                  <input type="tel" placeholder="Mobil Nömrəniz" value={phone} onChange={(e) => setPhone(e.target.value)}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-100 focus:border-orange-200 rounded-xl outline-none font-bold text-sm placeholder:text-gray-300" />
                  <input type="text" placeholder="Doğum Tarixi (12.05.1998)" value={birthDate} onChange={(e) => setBirthDate(e.target.value)}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-100 focus:border-orange-200 rounded-xl outline-none font-bold text-sm placeholder:text-gray-300" />
                  <p className="text-[11px] text-[#FF8C00] font-bold flex items-center gap-1 px-1">
                  ...
                  <button onClick={() => setDeliveryType('urgent')}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[10px] font-bold text-gray-400">Sürətləndirilmiş hazırlıq</p>
                      </div>
                    </div>
                  ...
                  <button onClick={() => setDeliveryType('express')}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[10px] font-bold text-gray-400">Prioritet hazırlıq, eyni gün kuryer</p>
                      </div>
                    </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none font-bold text-sm min-h-[75px] focus:border-orange-200 transition-all placeholder:text-gray-300 resize-none"
                  />
                </div>
