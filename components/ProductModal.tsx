...
const ProductModal: React.FC<ProductModalProps> = ({ product, initialData, onClose, onAddToCart }) => {
  const [selectedVariantIndex, setSelectedVariantIndex] = useState(initialData?.variantIndex || 0);
  ...
  const modelName = selectedVariant.modelName || '-';
            ...
            <div className="p-5 md:p-7 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {variants.map((variant, idx) => {
                      const vModel = variant.modelName || '-';
                      const vColor = variant.colorName || '-';
                      ...
                      return (
                        <button
                          key={idx}
                          onClick={() => { if (!isVarOutOfStock) handleVariantChange(idx); }}
                        ...
                        >
                          <p className="text-[11px] font-black text-[#1A1A1A]">Model: {vModel}</p>
                          <p className="text-[11px] font-black text-[#1A1A1A]">Rəng: {vColor}</p>
                          <div className="flex items-center justify-between mt-1">
                            <span className="text-xs font-black text-[#FF8C00]">{vPrice.toFixed(2)} AZN</span>
                            <span className={`text-[9px] font-bold ${isVarOutOfStock ? 'text-red-500' : 'text-gray-500'}`}>
                              {isVarOutOfStock ? 'Bitib' : `${vStock} əd`}
                            </span>
                          </div>
                        </button>
                      );
              ...
              <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Seçilmiş variant</p>
                    <p className="text-sm font-black text-[#1A1A1A] mt-1">Model: {modelName}</p>
                    <p className="text-sm font-black text-[#1A1A1A]">Rəng: {colorName}</p>
                  </div>
                  {stock > 0 && stock <= 20 && (
                    <span className="text-[10px] font-black text-yellow-600 bg-yellow-50 px-2 py-1 rounded-full">
                      {stock} əd qalıb
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-2xl border border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black text-[#1A1A1A]">Hədiyyəlik bağlama?</span>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setIsGift(false)} className={`px-4 py-1.5 rounded-xl text-[11px] font-black transition-all outline-none ${isGift === false ? 'bg-[#1A1A1A] text-white' : 'bg-white text-[#1A1A1A] border border-gray-200'}`}>XEYR</button>
                  <button onClick={() => setIsGift(true)} className={`px-4 py-1.5 rounded-xl text-[11px] font-black transition-all outline-none ${isGift === true ? 'bg-[#FF8C00] text-white' : 'bg-white text-[#1A1A1A] border border-gray-200'}`}>BƏLİ</button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <button onClick={() => setDeliveryType('standard')} className={`w-full flex items-center justify-between p-3 rounded-2xl border-2 transition-all ${deliveryType === 'standard' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-100 bg-white'}`}>
                  <div className="flex items-center gap-2">
                ...
                <button onClick={() => setDeliveryType('urgent')} className={`w-full flex items-center justify-between p-3 rounded-2xl border-2 transition-all ${deliveryType === 'urgent' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-100 bg-white'}`}>
                  <div className="flex items-center gap-2">
                ...
                <button onClick={() => setDeliveryType('express')} className={`w-full flex items-center justify-between p-3 rounded-2xl border-2 transition-all ${deliveryType === 'express' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-100 bg-white'}`}>
                  <div className="flex items-center gap-2">
              ...
              {/* ENDİRİM */}
              <section id="customer-type-section" className={`p-3 rounded-2xl space-y-2.5 border-2 transition-all ${showCustomerTypeError && customerType === null ? 'border-red-400 bg-red-50' : 'border-orange-200 bg-gradient-to-br from-orange-50 to-white'}`}>
                <div className="flex items-center gap-2">
                  <Sparkles className="h-3.5 w-3.5 text-[#FF8C00]" />
                  <h4 className="text-xs font-black text-[#1A1A1A]">Sizə özəl 10% endirim</h4>
                </div>
                <div className="flex gap-2.5">
                  <label onClick={() => { setCustomerType('new'); setShowCustomerTypeError(false); }} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl border-2 cursor-pointer transition-all ${customerType === 'new' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-200 bg-white hover:border-orange-200'}`}>
                  ...
                  <label onClick={() => { setCustomerType('loyal'); setShowCustomerTypeError(false); }} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl border-2 cursor-pointer transition-all ${customerType === 'loyal' ? 'border-[#FF8C00] bg-orange-50' : 'border-gray-200 bg-white hover:border-orange-200'}`}>
                ...
                {customerType && <p className="text-[10px] font-black text-[#FF8C00] text-center">{customerType === 'new' ? '10% endirim 🎉' : '10% endirim ❤️'}</p>}
                {showCustomerTypeError && customerType === null && <p className="text-[10px] font-black text-red-500 text-center">⚠️ Zəhmət olmasa seçin</p>}
              </section>

              {/* FORM */}
              <div className="border-t-2 border-dashed border-gray-200 pt-4 space-y-3">
                <div className="space-y-1.5">
                  <p className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-widest flex items-center gap-1.5"><Edit3 className="h-3 w-3 text-[#FF8C00]" /> Məhsul üzərinə yazı</p>
                ...
                <div className="space-y-1.5">
                  <p className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-widest flex items-center gap-1.5"><FileText className="h-3 w-3 text-[#FF8C00]" /> Əlavə Qeyd</p>
                ...
                <div className="space-y-1.5">
                  <p className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-widest flex items-center gap-1.5"><User className="h-3 w-3 text-[#FF8C00]" /> Müştəri Məlumatları</p>
                ...
                <div className="space-y-1.5">
                  <p className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-widest flex items-center gap-1.5"><MapPin className="h-3 w-3 text-[#FF8C00]" /> Çatdırılma Ünvanı</p>
              ...
              <div className="sticky bottom-0 bg-white pt-3 pb-2 border-t border-gray-100 space-y-3">
                <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-2xl px-3 py-2">
                  ...
                  <p className="text-[10px] font-bold text-amber-800">Sifarişin <span className="font-black">50%-i</span> kart-karta ön ödəniş tələb olunur.</p>
                </div>

                <div className="bg-[#1A1A1A] p-5 rounded-[2rem] flex items-center justify-between text-white shadow-2xl">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">YEKUN {getLoyaltyDiscount() > 0 && <span className="text-green-400">(-{getLoyaltyDiscount().toFixed(2)} AZN)</span>}</span>
                    <div className="flex items-baseline gap-2">
                      <span className="text-2xl font-black text-[#FF8C00]">{calculateTotal().toFixed(2)} AZN</span>
                      {(discountPrice || getLoyaltyDiscount() > 0) && <span className="text-xs font-bold text-gray-500 line-through">{price.toFixed(2)} AZN</span>}
                    </div>
                    <span className="text-[9px] text-amber-400 font-bold mt-0.5">Ön ödəniş: {(calculateTotal() * 0.5).toFixed(2)} AZN</span>
                  </div>
                  <button onClick={handleAddToCartClick} disabled={!isFormValid || isOutOfStock} className="bg-[#FF8C00] text-white px-6 py-4 rounded-full font-black text-sm flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-xl disabled:opacity-30 disabled:pointer-events-none outline-none">
